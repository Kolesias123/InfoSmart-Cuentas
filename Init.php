...
// - $needle: El valor a eliminar.
// - $haystack: El array.
// - $all: ¿Eliminar todas las coincidencias?
// Agradecimientos: http://php.net/manual/es/function.array-search.php#108979
function array_delete($needle, $haystack, $all = true) 
{
    ...
    foreach ($haystack as $num => $val) 
    {
        ...
        else if($val == $needle && $all === false && !isset ($done)) 
        {
            $done = true;
            unset($haystack_updated [$num]);
        }
