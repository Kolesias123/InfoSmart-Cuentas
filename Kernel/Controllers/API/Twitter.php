...
class Twitter
{
	...
	static function API($user = true)
	{
		...
		if($user)
		{
			...
			if(
				$verify->error == 'Could not authenticate you.' OR 
				$verify->errors[0]->message == 'Bad Authentication data' OR
				$verify->errors[0]->message == 'Invalid or expired token'
			)
				self::LogIn();
		}
	...
	// - $params (array): Parametros a enviar.
	// Más información: https://dev.twitter.com/docs/api
	static function Get_Id_($id, $type, $action, $params = array())
	{
		$API 	= self::API();
	...
	// - $params (array): Parametros a enviar.
	// Más información: https://dev.twitter.com/docs/api
	static function Publish_Id_($id, $type, $action, $params = array())
	{
		$API 	= self::API();
	...
	static function Publish_Status($status, $replyId = '')
	{
		...
		try
		{
			$result = $API->post('statuses/update', array(
				'status'				=> $status,
